...
class Usps {
    ...
    public function validatePickupAvailability($from_address, $time) {
        ...
        // create xml request
        $xml_usps = new SimpleXMLElement("<CarrierPickupAvailabilityRequest USERID=\"$this->user_id\"/>");
        ...
        try {
            ...
            // create usps response
            $xml_response = new SimpleXMLElement($usps_response);

            $error_message = 'Failed to get availability';
            // loop through children to find first address
            foreach($xml_response->children() as $key => $xml_value) {

                if ($key == 'DayOfWeek') {
                    $availability['day_of_week'] = (string) $xml_value;
                }
                if ($key == 'Date') {
                    $availability['date'] = Functions::convertTimeToMysql(strtotime((string) $xml_value));
                }
                if ($key == 'CarrierRoute') {
                    $availability['carrier_route'] = (string) $xml_value;
                }
                if ($key == 'Description') {
            ...
            if (!isset($availability['day_of_week'])) return $response->setFailure($error_message, 'USPS_VALIDATION_ERROR', 'USPS_VALIDATION_ERROR');
        
            $availability['carrier'] = 'usps';

            $response->set('availability', $availability);
